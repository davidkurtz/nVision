...
CREATE TABLE PSTREESELCTL (SETID VARCHAR2(5) NOT NULL,
...
/
CREATE UNIQUE  iNDEX PS_PSTREESELCTL ON PSTREESELCTL (SETID,
   SETCNTRLVALUE,
   TREE_NAME,
 ...
 NOLOGGING
/
ALTER INDEX PS_PSTREESELCTL NOPARALLEL LOGGING
/

REM GRANT SELECT, INSERT, UPDATE, DELETE ON pstreeselctl TO SYSADM;

CREATE OR REPLACE TRIGGER xx_pstreeselctl_inc
AFTER INSERT OR UPDATE OR DELETE ON pstreeselctl
FOR EACH ROW
BEGIN
  IF deleting THEN
